...
export default function Header(props) {
    ...
    const displayMenuLinks = menuLinks.map(link => <Link key={link.id} to={`${link.to}`} className="relative after:content-[''] after:absolute after:left-0 after:bottom-0 after:w-full after:h-0.5 after:bg-red-500 after:opacity-0 hover:after:opacity-100 after:transition-opacity after:duration-300 after:delay-200 pb-[5px] text-base dark:text-gray-200 tracking-wide leading-5">{link.text}</Link>) 
    
    return (
        <header className="py-3 px-4 sm:px-8 supports-backdrop-blur:bg-[hsl(240_10%_3.9%_/_0.6)] fixed top-0 z-50 w-full border-b border-gray-500 backdrop-blur">
            <nav className="navbar flex justify-between items-center">
                ...
                <div className="flex"> 
                    ...
                    <button className={`h-9 w-9 p-2 ${props.parentTheme === 'light' ? 'hover:bg-btn-white' : 'hover:bg-btn-dark'} transition-colors duration-150 ease-[cubic-bezier(0.4,0,0.2,1)] rounded-md`}>
