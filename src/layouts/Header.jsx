...
export default function Header() {
    ...
    const displayMenuLinks = menuLinks.map(link => <Link key={link.id} to={`${link.to}`} className="relative after:content-[''] after:absolute after:left-0 after:bottom-0 after:w-full after:h-0.5 after:bg-red-500 after:opacity-0 hover:after:opacity-100 after:transition-opacity after:duration-300 after:delay-200 pb-[5px] text-base dark:text-gray-200 tracking-wide leading-5 cursor-pointer text-center">{link.text}</Link>) 
    
    return (
        <header className="py-3 px-4 sm:px-8 supports-backdrop-blur:bg-[hsl(240_10%_3.9%_/_0.6)] fixed top-0 left-0 z-50 w-full border-b border-gray-500 backdrop-blur mb-[69px] flex justify-center md:max-w-[1400px]">
